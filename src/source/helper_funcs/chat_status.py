...
def is_whitelist_plus(chat: Chat, user_id: int, member: ChatMember = None) -> bool:
    return any(user_id in user for user in [WOLVES, TIGERS, DEMONS, DRAGONS, DEVS])


def is_disasters(chat: Chat, user_id: int, member: ChatMember = None) -> bool:
    return user_id in DEMONS or user_id in DRAGONS or user_id in DEVS


def is_sudo_plus(chat: Chat, user_id: int, member: ChatMember = None) -> bool:
    return user_id in DRAGONS or user_id in DEVS


def is_TheRiZoeL(chat: Chat, user_id: int, member: ChatMember = None) -> bool:
    if (
        chat.type == "private"
...
def is_Hentai_Lover_Boy(chat: Chat, bot_id: int, bot_member: ChatMember = None) -> bool:
...
def disasters(func):
    @wraps(func)
    def is_disasters_func(update: Update, context: CallbackContext, *args, **kwargs):
        bot = context.bot
        user = update.effective_user
        chat = update.effective_chat

        if user and is_disasters(chat, user.id):
            return func(update, context, *args, **kwargs)
...
def TheRiZoeL(func):
    ...
    def is_admin(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if user and is_TheRiZoeL(chat, user.id):
            return func(update, context, *args, **kwargs)
        if not user:
            pass
        elif DEL_CMDS and " " not in update.effective_message.text:
...
def user_not_admin(func):
    ...
    def is_not_admin(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if user and not is_TheRiZoeL(chat, user.id):
            return func(update, context, *args, **kwargs)
        if not user:
            pass
...
def Hentai_Lover_Boy(func):
    ...
    def is_admin(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if is_Hentai_Lover_Boy(chat, bot.id):
            return func(update, context, *args, **kwargs)
...
from src.source import connection
