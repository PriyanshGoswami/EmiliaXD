...
from src.__main__ import STATS, TOKEN, USER_INFO
...
from src.source.helper_funcs.extraction import extract_user
...
def set_about_me(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if repl_user_id in [bot.id, 777000, 1087968824] and (user_id in DEVS):
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id in [777000, 1087968824] and sender_id not in DEVS:
            message.reply_text("You are not authorised")
            return

        if user_id == bot.id and sender_id not in DEVS:
            message.reply_text(
