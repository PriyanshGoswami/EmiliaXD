...
from src.source.helper_funcs.extraction import extract_user_and_text
from src.source.helper_funcs.string_handling import extract_time
from src.source.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if message.reply_to_message and message.reply_to_message.sender_chat:
        ...
        if r:
            message.reply_text("Channel {} was expelled successfully from {}".format(
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEVS:
        if user_id == Owner:
    ...
    try:
        ...
        reply = (
            f"Another one bites the dust...! \n Banned {mention_html(member.user.id, html.escape(member.user.first_name))}."
        )
        if reason:
...
def unbanb_btn(update: Update, context: CallbackContext) -> str:
    ...
    if query.data != "unbanb_del":
        ...
        if query_match == "unbanb_unban":
            user_id = splitter[1]
            if not is_TheRiZoeL(chat, int(user.id)):
...
def punchme(update: Update, context: CallbackContext):
    ...
    if is_TheRiZoeL(update.effective_chat, user_id):
