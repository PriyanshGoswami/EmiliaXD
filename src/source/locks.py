import src.source.sql.locks_sql as sql
import html
import ast
...
from src import dispatcher, DRAGONS, LOGGER
...
def unlock(update, context) -> str:
    ...
    if is_TheRiZoeL(chat, message.from_user.id):
        if len(args) >= 1:
            ltype = args[0].lower()
            if ltype in LOCK_TYPES:
                # Connection check
        ...
        if (
            ...
            if lockable == "bots":
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        if not is_Hentai_Lover_Boy(chat, context.bot.id):
                            send_message(
                                update.effective_message,
