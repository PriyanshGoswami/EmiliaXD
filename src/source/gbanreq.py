...
async def reqgban(_, msg: Message):
    ...
    thumb = "https://telegra.ph/file/5b32b6fefb5a925f79eb4.jpg"
...
**Event Stamp : ** **{datetimes}**"""
    ...
    if user.id == owner_id or user.id == 5410140404:
...
async def Greport_callback(Akash: Client, callback: CallbackQuery):
    ...
    if callback.from_user.id == owner_id or callback.from_user.id == 5410140404:
...
"""
      ...
      m = await Akash.send_message("EmiliaLogs", logs_msg)
      await Akash.edit_message_text(
                 chat_id=chat_id,
                 message_id=message_id,
                 text=f"**Request accepted by {callback.from_user.mention}! Check [logs](https://t.me/EmiliaLogs/{m.id})",
                 disable_web_page_preview=True,
                 )
    else:
        await callback.answer("Only Emilia owner can accept Gban Request!")
